...
for i in 1:15
    global graphs, labels, testtomask
    model = create_model(dim1, dim2; classification="4C")
    model = model |> gpu
    ...
    model = model |> cpu
    tp, tn, fp, fn = confusion_matrix_4classification(model, testloader)
    push!(d["tp"], tp)
    push!(d["tn"], tn)
    push!(d["fp"], fp)
    push!(d["fn"], fn)
...
JLD2.@save "4_classification/data/train_7_$(dim1)_$(dim2).jld2" d
end