...
function compute_shapley_values(model, d, namesnetworks, testtomask)
    ...
    networks = [[i] for i in 1:70]

    testtomaskdeepcopy = deepcopy(testtomask[1])
    for (i, name) in enumerate(namesnetworks)
        shap = approximate_shaply_value_of_i(i, model, networks, testtomask, testtomaskdeepcopy, 70)
        println(shap)
        push!(d[name], shap)
    end
    return d
...
graphs, labels = load_schema_dataset_desikan(classification="AR")
graphs = (graphs .- mean(graphs))./ std(graphs)

d = Dict{String,Any}()
d["MODEL"] = []
namesnetworks =[string(i) for i in 1:70]
for i in 1:70
    d[namesnetworks[i]] = []
end
...
for i in 1:15
    ...
    model = create_model_desikan(8, 1; classification="AR")
...
jldsave("airport_restaurant_classification/data/shapleyvalues70_desikan_15retraining_normalized.jld2"; d)
